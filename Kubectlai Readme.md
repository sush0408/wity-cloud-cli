...
## Wity Cloud Deploy - Enhanced Database Deployments

This repository has been enhanced with comprehensive database deployment capabilities, including a robust **Percona MongoDB** deployment with operator-based management, monitoring, and backup solutions.

> 📚 **Complete Documentation**: See the [docs/](docs/) folder for comprehensive guides, architecture diagrams, and troubleshooting information.

### Percona MongoDB Deployment

The enhanced Percona MongoDB deployment provides:
...
Deploy the complete Percona MongoDB stack:
...
- **Sharded MongoDB cluster** with 2 shards (rs0, rs1) and 3 replica set members each
- **3 Config Servers** for metadata and configuration management
- **2 Mongos Routers** for query routing and load balancing
- **PMM Server** for monitoring and performance insights
- **Automated daily backups** with compression and retention management
- **Network components** (MetalLB, cert-manager) for production readiness
...
# Visit: http://localhost:8080 (admin/admin-password)
```

For detailed documentation, see [docs/PERCONA_MONGODB_README.md](docs/PERCONA_MONGODB_README.md).

### Other Database Options

The deployment framework also supports:
...
Run `./databases.sh` to see all available database deployment options.

### Monitoring Stack

Enhanced monitoring capabilities include:

- **PMM Server** for Percona database monitoring
- **Prometheus** and **Grafana** for general monitoring
- **Loki** for log aggregation
- **AlertManager** for alerting

Run `./monitoring.sh` to deploy monitoring components.
